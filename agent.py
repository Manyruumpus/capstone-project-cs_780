...
def policy(obs, rng=None):
    global _model, _last_obs, _last_rng_id
    _load_once()

    current_rng_id = id(rng) if rng is not None else None
    ...
    if not np.any(obs[:17]) and _last_obs is not None:
        obs_to_use = _last_obs
    else:
        obs_to_use = obs
        if np.any(obs[:17]): # Update memory only if we actually see the box
            _last_obs = obs.copy()

    x = torch.tensor(obs_to_use, dtype=torch.float32).unsqueeze(0)
