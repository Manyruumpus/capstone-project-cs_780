...
OBS_DIM = 18
STACK_N = 4
STACKED_DIM = OBS_DIM * STACK_N

class DQN(nn.Module):
    def __init__(self, in_dim: int = STACKED_DIM, n_actions: int = 5):
        super().__init__()
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    global _last_action, _repeat_count, _obs_hist, _last_rng_id
    _load_once()

    obs = np.asarray(obs, dtype=np.float32)

    if (_last_rng_id is None) or (id(rng) != _last_rng_id):
        _obs_hist = deque([obs.copy()] * STACK_N, maxlen=STACK_N)
        _last_action = None
        _repeat_count = 0
        _last_rng_id = id(rng)
    elif _obs_hist is None:
        _obs_hist = deque([obs.copy()] * STACK_N, maxlen=STACK_N)
    else:
        _obs_hist.append(obs.copy())

    stacked_obs = np.concatenate(list(_obs_hist), axis=0).astype(np.float32, copy=False)
