...
class OBELIX:
    ...
    def update_reward(self):
        ...
        # OFFICIAL DENSE REWARDS (Simplified weights)
        # Give these EVERY step the sensor is active, not just once.
        if np.any(sensor_bits):
            # Forward near is +3, Far is +2, IR is +5
            if self.sensor_feedback[16]: reward += 5.0 # IR
            if np.any(self.sensor_feedback[4:12]): reward += 3.0 # Forward sensors
            if np.any(self.sensor_feedback[:4]) or np.any(self.sensor_feedback[12:16]):
                reward += 1.0 # Side sensors
        else:
            reward -= 1.0 # Only penalize if nothing is seen

        # Aggressive Stuck Penalty
        if bool(self.sensor_feedback[17]):
            reward -= 50.0 # Reduce from -200 to -50 during training so the agent isn't too "scared" to move
